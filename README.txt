Student ID: F74031166

A:
after entered "nm test"(compiled from testA.cpp):
...
0000000000400624 T _Z7averagePdRd
may means function"double average (pointer double variable,reference double variable)"

0000000000400652 T _Z7averageif
may means function"int average(int variable,float variable)"

000000000040067a T main
may means function"main()"
...
first column means the size of variable"char int float double"(bytes)
as we know char is 1 bytes , int is 4,float is 4,double is 8
but pointer stores address which means the size of the pointer depends on
the OS (my OS is 64 bit =8bytes);
